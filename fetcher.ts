              ...
              if (typeof xdcc !== "undefined") {
                ...
                if (tmp !== null) {
                  let ep: string | number = tmp[0]
                    ...
                    .replace("-", ".");
                  ep = parseFloat(ep);
                  console.log("\x1b[36m%s\x1b[0m", `job number: ${index}`);
                  this.shows.push({
                    id: res.id,
                    name: res.name,
                    ep: ep,
    ...
    try {
      ...
      if (typeof eval(parsed) === "object") {
        ...
        res = res.sort((item1, item2) => {
          if (item1.bot.includes("NEW") && item2.bot.includes("Sensei")) {
            return -1;
          }
          if (item1.bot.includes("Sensei") && item2.bot.includes("ARCHIVE")) {
            return -1;
          }
          if (item1.bot.includes("NEW") && item2.bot.includes("ARCHIVE")) {
            return -1;
          }
          if (item1.bot.includes("HOLLAND") && !item2.bot.includes("HOLLAND")) {
            return -1;
          }
          return 1;
        });
