...
db.defaults({ shows: [], index: [], watched: [], follows: [], settings: [] }).write()
...
try {
    ...
    app.whenReady().then(() => {
        ...
        // show load spinner
        win.loadFile(path.normalize(path.resolve(__dirname, '../views', 'loading.html'))).then(() => { win.show()})
        // start xdccserver
        ...
        if(firstBoot) {
            let data:any = db!.get('settings')
            data.find({id:0}).assign({startupCheck: false}).write()
            data = null
        } 
        db = null
        // extending express
        serv.app.use('/static', express.static(path.normalize(path.resolve(__dirname, '../node_modules'))))
        ...
        serv.app.get('/search', (req: Request, res: Response) => {
            needle('get', `https://myanimelist.net/search/prefix.json?type=all&keyword=${req.query.keyword}`, {json: true, compressed: true}).then((resp) => {
                let json = resp.body.categories.filter((cat: { type: string }) => cat.type === 'anime')[0].items
        ...
        serv.on('http-ready', () => {
            ...
            ipcMain.on('data-search', (_event, arg) => {
                MAL.hide()
                MAL.loadFile(`${__dirname}/views/loading.html`).then(() => { MAL.show()}).finally(() => {
            ...
            ipcMain.on('settings-show', (_event, arg) => {
                winSettings.loadURL(`http://localhost:3000/settings`).then(()=>winSettings.show())
